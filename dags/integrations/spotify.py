...
class SpotifyAPI:
    ...
    def create_playlist(self, playlist_name, spotify_album_ids):
        all_tracks = [track for album_id in spotify_album_ids for track in self.sp.album_tracks(album_id).pop("items")]
        existing_playlists = self.sp.user_playlists(self.user).pop("items")
        if any([playlist["name"] == playlist_name for playlist in existing_playlists]):
            matched_playlists = [playlist for playlist in existing_playlists if playlist["name"] == playlist_name]
            ...
            playlist_tracks = {x["track"]["id"] for x in self.sp.user_playlist_tracks(self.user, playlist["id"]).pop("items")}
        ...
        self.sp.user_playlist_add_tracks(self.user, playlist["id"], [track["id"] for track in all_tracks])
